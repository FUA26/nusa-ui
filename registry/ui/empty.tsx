...
const emptyMediaVariants = cva("[&_svg]:pointer-events-none [&_svg]:shrink-0", {
  variants: {
    variant: {
      ...
      icon: "flex size-12 items-center justify-center rounded-full bg-muted [&_svg:not([class*='size-'])]:size-6",
    },
  },
  defaultVariants: {
    variant: "default",
...
function EmptyDescription({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <p
      ...
      className={cn(
        "text-sm text-balance text-muted-foreground [&_a]:text-foreground [&_a]:underline [&_a]:underline-offset-4 [&_a]:hover:text-primary",
        className
      )}
      {...props}
    />
